...
# FIX: agregado EVELIN (sin Y) que es como aparece en algunas filas del sheet
...
def cargar_ventas_diarias():
    try:
        ...
        rename = {
            ...
            # FIX: nombres reales en el sheet actual
            'DEPOSITOS':'Cierres',
            'DEPÓSITOS':'Cierres',
            'PRESUPUESTADO':'Venta Dia Siguiente',
            # FIX: nuevas columnas de leads detectadas en el sheet
        ...
        # FIX: incluir todas las columnas de leads en la limpieza numérica
        for col in ['Valoraciones','Leads WPP','Leads IG','Leads Formulario',
...
with tab1:
    ...
    if df_filtrado.empty:
        ...
        cols_num = ['Leads WPP','Leads IG','Formulario','Landing','TikTok','Valoraciones','Presupuestado','Depósitos']
        totales = {c: df_show[c].sum() if c in df_show.columns and c in cols_num else '' for c in df_show.columns}
        totales['Fecha'] = '📊 TOTAL'
        totales['Día'] = ''; totales['Semana'] = ''; totales['Responsable'] = ''; totales['Grupo'] = ''
        ...
        def render_embudo_horizontal(titulo, etapas_vals, color_borde):
            ...
            for i, (etapa, val) in enumerate(etapas_vals):
                ...
                <div style="display:inline-flex;flex-direction:column;align-items:center;justify-content:center;
                            ...
                            text-align:center;vertical-align:top;box-shadow:0 4px 15px rgba(0,0,0,0.3)">
                ...
                if i < len(etapas_vals) - 1:
                    items_html += f'<div style="display:inline-flex;align-items:center;justify-content:center;padding:0 6px;vertical-align:top;margin-top:30px"><span style="color:{color_borde};font-size:1.5rem;font-weight:300">→</span></div>'
        ...
        # FIX: suma todas las fuentes de leads
        ...
        etapas_esp = [
            ...
            ("⭐ Valoración", s(df_esp_emb,'Valoraciones')), ("💵 Presupuesto", s(df_esp_emb,'Venta Dia Siguiente')),
            ("💳 Financiamiento", "—"), ("🏥 Val. Presencial", "—"),
            ("📅 Ag. Depósito", s(df_esp_emb,'Cierres')), ("✅ Venta Cerrada", "—"),
        ]
        etapas_usa = [
            ("📥 Leads", leads_usa), ("📞 Contactado", "—"), ("🔇 No Contesta", "—"),
            ("💻 Val. Virtual", s(df_usa_emb,'Valoraciones')), ("💵 Presupuesto", s(df_usa_emb,'Venta Dia Siguiente')),
            ("🏥 Ag. Presencial", "—"), ("📅 Ag. Depósito", s(df_usa_emb,'Cierres')),
        ]
        ...
        if not df_tareas.empty:
            ...
            # FIX: incluir EVELIN en asesores
        ...
        with ag1:
            st.markdown("<div style='color:#7c6af7;font-weight:800;font-size:0.9rem;margin-bottom:8px'>🇺🇸 USA</div>", unsafe_allow_html=True)
            if not df_ag_usa.empty: st.dataframe(df_ag_usa, use_container_width=True, hide_index=True)
            else: st.info("Sin agenda pendiente USA.")
        with ag2:
            st.markdown("<div style='color:#00d4aa;font-weight:800;font-size:0.9rem;margin-bottom:8px'>🇪🇸 España</div>", unsafe_allow_html=True)
            if not df_ag_esp.empty: st.dataframe(df_ag_esp, use_container_width=True, hide_index=True)
            else: st.info("Sin agenda pendiente España.")
...
with tab2:
    ...
    try:
        ...
        if df_m.empty:
            ...
            df_esp_m2 = df_m[df_m['Grupo_Pais']=='España'] if 'Grupo_Pais' in df_m.columns else pd.DataFrame()
            df_usa_m2 = df_m[df_m['Grupo_Pais']=='USA']    if 'Grupo_Pais' in df_m.columns else pd.DataFrame()
            ...
            def barras_asesor(df_grupo, titulo, color_borde):
                ...
                if not df_hoy_g.empty and 'Responsable' in df_hoy_g.columns:
                    ...
                    for _, row in df_r.iterrows():
                        ...
                        c = int(row['Cierres']); faltan = max(META_DIARIA - c, 0)
                        ...
                        <div style="background:linear-gradient(135deg,#0d0d0d,#1a1500);border:1px solid {color_borde};
                            ...
                            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
                                <div style="color:white;font-weight:700">👤 {row['Responsable']}</div>
            ...
            with ec1: tarjeta_meta("Depósitos Hoy",    get_cierres(df_esp_m2, hoy_date, hoy_date),              META_DIARIA*n_esp,  "📅", "#00d4aa")
            with ec2: tarjeta_meta("Depósitos Semana", get_cierres(df_esp_m2, inicio_sem.date(), hoy_date),     META_SEMANAL*n_esp, "📆", "#00d4aa")
            with ec3: tarjeta_meta("Depósitos Mes",    get_cierres(df_esp_m2, inicio_mes.date(), hoy_date),     META_MENSUAL*n_esp, "🗓️", "#00d4aa")
            barras_asesor(df_esp_m2, "Progreso de HOY — España", "#00d4aa")
            st.markdown("---")
            ...
            with uc1: tarjeta_meta("Depósitos Hoy",    get_cierres(df_usa_m2, hoy_date, hoy_date),              META_DIARIA*n_usa,  "📅", "#7c6af7")
            with uc2: tarjeta_meta("Depósitos Semana", get_cierres(df_usa_m2, inicio_sem.date(), hoy_date),     META_SEMANAL*n_usa, "📆", "#7c6af7")
            with uc3: tarjeta_meta("Depósitos Mes",    get_cierres(df_usa_m2, inicio_mes.date(), hoy_date),     META_MENSUAL*n_usa, "🗓️", "#7c6af7")
            barras_asesor(df_usa_m2, "Progreso de HOY — USA", "#7c6af7")
            st.markdown("---")
            ...
            if not df_mes_all.empty and 'Responsable' in df_mes_all.columns:
                ...
                fig_rank = px.bar(df_rank, x='Responsable', y='Depósitos', color='Grupo',
                                  color_discrete_map={'España':'#00d4aa','USA':'#7c6af7','Por Clasificar':'#f0d080'})
